...
const USER_SESSION_KEY = "kanselo-mentor-admin";
const EXPIRY_TIME = process.env.NODE_ENV === "development" ? 1 * 60 * 60 : 300;
const EXPIRY_TIME_MILISECONDS = EXPIRY_TIME * 1000;
...
export async function setCookie(userData: UserDataAndAccessToken) {
  const expires = new Date(Date.now() + EXPIRY_TIME_MILISECONDS);
...
export async function updateSession(request: NextRequest) {
  ...
  user.expires = new Date(Date.now() + EXPIRY_TIME_MILISECONDS);
